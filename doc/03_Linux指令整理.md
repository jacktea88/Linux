...
* ![1788581737830](image/03_Linux指令整理/1788581737830.png)
* ![1788582035946](image/03_Linux指令整理/1788582035946.png)
...
* **過濾器與文字處理：**
  * `grep`：搜尋特![1788581732834](image/03_Linux指令整理/1788581732834.png)定字串（`-n` 行號，`-c` 統計次數，`-E` 支援擴充正規表示式）。
...
* **帳號與群組管理：**
  * `useradd`：新增帳號（`-G` 加入次要群組，`-s` 指定 Shell，`-m` 建立家目錄）。

    例：useradd  -m -s  /bin/bash  -G sudo  {帳號名稱}**(注意：記得設密碼)**
...
* **權限變更：**
  * `chmod`：變更權限（支援符號模式如 `u+x` 或數字模式如 `755`）。
    - 符號模式：u(user)、g(group)、o(others)、a(all)
      例：chmod  a+rwx (file or folder)
    - 數字模式：r=4，w=2，x=1
      例：chmod 777 (file or folder)
  * `chown`：變更檔案擁有者與群組（`-R` 遞迴地改變檔案目錄）。
  * `chgrp`：變更檔案擁有群組。
